...
# Extrai a tag atual
TG=$(cat docker-compose.yml | grep otavio27/joinbus: | awk -F':' '{print $3}')
...
sed -i "s|otavio27/joinbus:${TG}|otavio27/joinbus:${new_tag}|g" docker-compose.yml
...
if [ $? -eq 1 ]; then
    ...
    docker build -t otavio27/joinbus:${new_tag} . && clear
    docker push otavio27/joinbus:${new_tag} && sleep 2s && clear
else
    docker build -t otavio27/joinbus:${new_tag} . && clear
    docker push otavio27/joinbus:${new_tag} && sleep 2s && clear
fi
